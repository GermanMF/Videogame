...
function preload(){
	juego.load.image("fondo","assets/fondo5.jpg",800,600);
	juego.load.image("boton","assets/start-button.png",104,58);
	juego.load.spritesheet("bola", "assets/frijol.png",34,20);
	juego.load.image("barra","assets/barra.png",100,20);
	juego.load.image("barra2","assets/grano3.png",100,20);	
	...
	juego.load.image("piso","assets/piso_1.png",64,16); //Normal
	juego.load.image("pisogrande","assets/piso_3.png",64,16); //Grande
	juego.load.image("pisochico","assets/piso_2.png",64,16); //Chica
	...
	juego.load.image("poder","assets/estrellag.png",24,22);
	juego.load.image("poderM","assets/cocafeliz.png",24,22);
	...
	for(var i=0;i<6; i++){
		for(j=0;j<10;j++){
			if(i==0 || i == 2 || i==4){
				...
				contador++;
				var barras = barra.create(50+(j*110), 100+(i*30),"barra2");	
			}
...
function rebote (bola,barra) {
	...
	if(contador == 0){//Ganaste
		Estado = juego.add.text(juego.world.centerX, 400, '¡Ganaste!\nPresiona cualquier tecla para continuar', { font: "40px Arial", fill: "#000", align: "center" })
